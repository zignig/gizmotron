...
def main_parser(parser=None):
    ...
    p_boneload.add_argument(
        "-p", "--port", type=str, required=True, help="serial port to program over"
    )
    p_boneload.add_argument(
        "-r", "--ram", action="store_true", help="download to ram only"
    )
...
def main_runner(parser, args, maker, fw=None, ports=(), build_args={}):
    if args.action == "simulate":
        ...
        with pysim.Simulator(
            fragment, vcd_file=args.vcd_file, gtkw_file=args.gtkw_file, traces=ports
        ) as sim:
