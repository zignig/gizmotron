# lifted from https://github.com/awygle/nmigen/blob/streams/nmigen/lib/stream.py
# testing integration with the boneless
...
class StreamSource:
    ...
    def from_sink(
        cls, sink
    ):  # should be typed to take StreamSink and return StreamSource
        payload_type = convert_direction(
            sink.payload_type, Direction.FANIN, Direction.FANOUT
        )
        return StreamSource(payload_type, sink.sop_enabled, sink.eop_enabled)
    ...
    def __init__(
        ...
        self.payload_type = convert_direction(
            payload_type, Direction.NONE, Direction.FANOUT
        )
        ...
        for (name, shape, dir) in self.payload_type:
            if dir != DIR_FANOUT:
                raise TypeError(
                    "Field {!r} of StreamSource payload type has invalid direction: should be "
        ...
        if sop:
            self.sop: OutputSignal = OutputSignal(Signal())

        if eop:
            self.eop: OutputSignal = OutputSignal(Signal())
...
class StreamSink:
    @classmethod
    def from_source(cls, source: StreamSource):  # should be typed to return StreamSink
        payload_type = convert_direction(
            source.payload_type, Direction.FANOUT, Direction.FANIN
        )
        return StreamSink(payload_type, source.sop_enabled, source.eop_enabled)
    ...
    def __init__(
        ...
        self.payload_type = convert_direction(
            payload_type, Direction.NONE, Direction.FANIN
        )
    ...
    def connect(self, source: StreamSource):
        ...
        for (
            (sinkname, sinkshape, sinkdir),
            (sourcename, sourceshape, sourcedir),
        ) in zip(self.payload_type, source.payload_type):
            if (
                sinkname != sourcename
                or sinkshape != sourceshape
                or sinkdir != DIR_FANIN
                or sourcedir != DIR_FANOUT
            ):
                raise TypeError(
                    "Stream source payload type {!s} does not match stream sink payload type {!s}".format(
                        self.payload_type, source.payload_type
                    )
                )

        ops = []

        ops += [source.ready.eq(self.ready), self.valid.eq(source.valid)]
...
class AsyncFIFOStream(Elaboratable):
    def __init__(
        self, payload_type: Layout, depth: int, sop: bool = True, eop: bool = True
    ):
        self.source = StreamSource(payload_type, sop, eop)
        self.sink = StreamSink(payload_type, sop, eop)
...
class StreamReducer(Elaboratable):
    def __init__(self, input: StreamSource, width: int):
        ...
        self.source = StreamSink(
            Layout([("data", width, DIR_FANOUT)]), input.sop_enabled, input.eop_enabled
        )
