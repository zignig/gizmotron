...
def block():
    return "{", ZeroOrMore([amp, command, text]), "}"


def command():
    return esc, symbol, ZeroOrMore([block, text])


def comment():
    return _(r"424323423")
...
class Vis(PTNodeVisitor):
    ...
    def visit_amp(self, node, children):
        print("&->", node, children)
    ...
    def visit_command(self, node, children):
        print("command->", node, children)

    def visit_tex(self, node, children):
        print("tex->", node, children)
